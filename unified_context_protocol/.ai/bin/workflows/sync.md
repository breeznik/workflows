...
|---------|-------------|--------|
| **The Plan** | `.ai/context/active/PLAN.md` | **OVERWRITE** with current detailed plan & status. |
| **Progress** | `.ai/context/changelog.md` | **APPEND** completed steps since last sync. |
| **Learnings** | `.ai/knowledge/learnings.md` | **APPEND** new facts, patterns, or user preferences. |
...
FUNCTION sync_state():
  ...
  CURRENT_DISK_PLAN = READ_FILE ".ai/context/active/PLAN.md"
  IF CURRENT_DISK_PLAN differs significantly from INTERNAL_STATE.original_read:
     ...
     WRITE_FILE ".ai/context/active/PLAN.md" CONTENT = MERGED_PLAN
  ELSE:
     WRITE_FILE ".ai/context/active/PLAN.md" CONTENT = INTERNAL_STATE.current_plan

  # 3. Log Progress
  IF INTERNAL_STATE.has_new_completed_items:
    APPEND_FILE ".ai/context/changelog.md":
      CONTENT = "- [x] " + INTERNAL_STATE.just_completed_items

  # 4. Save Knowledge
  IF INTERNAL_STATE.learned_something_new:
    APPEND_FILE ".ai/knowledge/learnings.md":
...
## 3. Verification
- open `.ai/context/active/PLAN.md` -> Does it match your internal plan?
